...
- `backend/` is a bare FastAPI scaffold (`app/main.py` with a health endpoint). The `api/ core/ models/ schemas/ services/ repositories/ ai/ workers/` module layout is the Sprint 0 target, not a guarantee — check before assuming a directory exists.
- `frontend/` is a bare Next.js 15 App Router scaffold from `create-next-app`. React Query and shadcn/ui are not yet installed — check `package.json` before assuming they're available.
- `docker-compose.yml` and `.env.example` exist but are currently empty placeholders (Sprint 0, Issue #1 and #6 fill these in).
- No database migrations, no auth, no ingestion or review pipeline exist yet.
